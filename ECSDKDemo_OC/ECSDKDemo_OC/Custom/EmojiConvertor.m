...
@interface EmojiConvertor ()
@property (nonatomic, strong) NSArray *emojiMap;
@end

@implementation EmojiConvertor
+(EmojiConvertor*)sharedInstance{
    static dispatch_once_t emojiConvertor;
    static EmojiConvertor *emoji;
    dispatch_once(&emojiConvertor, ^{
        emoji = [[EmojiConvertor alloc] init];
    });
    return emoji;
}
- (id)init
{
    if (self = [super init]) {
        self.emojiMap = [NSArray arrayWithObjects:
...
- (NSArray *)getEmojisOfSoftbank
{
    ...
    for (NSArray *item in self.emojiMap) {
