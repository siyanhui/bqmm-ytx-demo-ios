...
+(CustomEmojiView*)shardInstance{
    ...
    dispatch_once(&emojiviewOnce, ^{
        cutomemojiview = [[CustomEmojiView alloc] initWithFrame:CGRectMake(0, 0, 320.0f, 216.0f)];
    });
...
-(instancetype)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        ...
        self.frame = CGRectMake(self.frame.origin.x, self.frame.origin.y, 320.0f, 216.0f);
        ...
        for (int p=0; p<pageCount; p++)
        {
            ...
            UIButton* delBtn = [[UIButton alloc] initWithFrame:CGRectMake(page_X+260.0f, 137.0f, 40.0f, 30.0f)];
