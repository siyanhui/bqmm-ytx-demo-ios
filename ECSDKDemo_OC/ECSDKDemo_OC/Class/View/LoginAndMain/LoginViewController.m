...
-(void)prepareUI {
    ...
    NSString *title = @"云通讯IM";
    CGSize size = [title sizeWithFont:[UIFont systemFontOfSize:14.0f]];
    UIView *titleView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, size.width+10, size.height)];
    ...
    UILabel	*titleText = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, size.width+20, size.height)];
    ...
    _userName = [[UITextField alloc]initWithFrame:CGRectMake(30.0f, frameY, 260.0f, 30.0f)];
    ...
    _password = [[UITextField alloc] initWithFrame:CGRectMake(30.0f, _userName.frame.origin.y+_userName.frame.size.height+15.0f, 260.0f, 30.0f)];
    _password.borderStyle = UITextBorderStyleLine;
    _password.placeholder = @"输入VoIP密码";
    ...
    if ([DemoGlobalClass sharedInstance].loginAuthType == LoginAuthType_NormalAuth) {
        ...
        _userName.placeholder = @"输入VoIP账号";
    }
    _nextBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    _nextBtn.frame =CGRectMake(10, textfile.frame.origin.y+textfile.frame.size.height+15.0f, 300, 45);
    [_nextBtn setBackgroundImage:[UIImage imageNamed:@"select_account_button"] forState:UIControlStateNormal];
