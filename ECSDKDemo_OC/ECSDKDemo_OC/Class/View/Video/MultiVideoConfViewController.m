...
- (void)loadView
{
    ...
    CGFloat height;
    CGFloat margin;
    if (range.size.height == 480) {
        margin = 8.0f;
        height = (range.size.height-CGRectGetMaxY(self.view4.frame)-margin*3)/3;
    } else {
        margin = 30.0f;
        height = 44.0f;
    }
    //创建控麦、切换摄像头和结束视频
    [self createButtonWithRect:CGRectMake(45-11,CGRectGetMaxY(self.view4.frame)+margin, height, height) andTag:VideoConfVIEW_BtnChangeCam andImageName:@"videoConf05"];
    [self createButtonWithRect:CGRectMake(147-11, CGRectGetMaxY(self.view4.frame)+margin, height, height) andTag:VideoConfVIEW_BtnMic andImageName:@"videoConf07"];
    [self createButtonWithRect:CGRectMake(11, range.size.height-height*2-margin, 298, height) andTag:VideoConfVIEW_BtnExit andImageName:@"videoConf58"];
