...
-(void)prepareUI
{
    ...
    self.tableView = [[UITableView alloc]initWithFrame:CGRectMake(0.0f, 50.0f, 320.0f, hight-110.0f) style:UITableViewStylePlain];
    
    if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 7.0)
    {
        self.tableView.frame = CGRectMake(0.0f, 50.0f, 320.0f, hight-110.0f);
    }
    else
    {
        self.tableView.frame = CGRectMake(0.0f, 50.0f, 320.0f, hight-110.0f-20.0f);
    }
    ...
    countLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, hight-60-64, 160, 60.0f)];
    ...
    [inviteBtn setBackgroundImage:[CommonTools createImageWithColor:[UIColor redColor]] forState:UIControlStateNormal];
    ...
    if (_selectedArray.count == 0) {
        if (_isDiscuss) {
            ...
            if ([viewController respondsToSelector:aSelector]) {
                ...
                setter(viewController, aSelector, self.groupId);
            }
        ...
        [[ECDevice sharedInstance].messageManager inviteJoinGroup:self.groupId reason:@"" members:inviteArray confirm:confirm completion:^(ECError *error, NSString *groupId, NSArray *members) {
            ...
            if(error.errorCode ==ECErrorType_NoError) {
                ...
                if ([viewController respondsToSelector:aSelector]) {
                    IMP aIMP = [viewController methodForSelector:aSelector];
                    void (*setter)(id, SEL, NSString*) = (void(*)(id, SEL, NSString*))aIMP;
                    setter(viewController, aSelector,strongSelf.groupId);
                }
                [strongSelf.navigationController setViewControllers:[NSArray arrayWithObjects:[strongSelf.navigationController.viewControllers objectAtIndex:0],viewController, nil] animated:YES];
