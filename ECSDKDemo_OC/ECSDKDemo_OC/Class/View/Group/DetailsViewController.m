...
- (void)viewDidLoad {
    ...
    self.isDiscuss = [[DemoGlobalClass sharedInstance] isDiscussGroupOfId:self.groupId];
    self.isDiscussOrGroupName = self.isDiscuss?@"讨论组":@"群组";
...
-(void)judgeSuccess:(ECError*)error {
    if (error.errorCode != ECErrorType_NoError) {
        ...
        NSString* detail = error.errorDescription.length>0?[NSString stringWithFormat:@"\r描述:%@",error.errorDescription]:@"";
        [self showToast:[NSString stringWithFormat:@"错误码:%d%@",(int)error.errorCode,detail]];
        return;
    }
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0) {
        ...
        if (cell == nil) {
            ...
            frame.origin.x = cell.contentView.frame.size.width-switchs.frame.size.width-20.0f;
    ...
    if (buttonIndex == 0) {
        ...
        [[ECDevice sharedInstance].messageManager setGroupMemberRole:self.groupId member:cell.member.memberId role:role completion:^(ECError *error, NSString *groupId, NSString *memberId) {
            ...
            if (error.errorCode == ECErrorType_NoError) {
                ...
                [strongSelf showToast:[NSString stringWithFormat:@"错误码:%d%@",(int)error.errorCode,error.description]];
            }
        }];
    } else if (buttonIndex==1) {

        __weak typeof(self)weakSelf = self;
        [[ECDevice sharedInstance].messageManager setGroupMemberRole:self.groupId member:cell.member.memberId role:ECMemberRole_Creator completion:^(ECError *error, NSString *groupId, NSString *memberId) {
            __strong __typeof(weakSelf)strongSelf = weakSelf;
            if (error.errorCode == ECErrorType_NoError) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    ECGroupMember *member1 = [strongSelf.tableViewArray objectAtIndex:0];
                    if ([member1.memberId isEqualToString:[DemoGlobalClass sharedInstance].userName]) {
                        member1.role = ECMemberRole_Member;
                    }
                    ECGroupMember *member2 = [strongSelf.tableViewArray objectAtIndex:cell.tag];
                    if ([member2.memberId isEqualToString:cell.memberId]) {
                        member2.role = ECMemberRole_Creator;
                    }
                    [strongSelf.tableViewArray sortedArrayUsingComparator:^(ECGroupMember *obj1, ECGroupMember* obj2){
                        if(obj1.role < obj2.role) {
                            return(NSComparisonResult)NSOrderedAscending;
                        }else {
                            return(NSComparisonResult)NSOrderedDescending;
                    ...
                    _isOwner = NO;
                    _isCreater = NO;
                    [strongSelf.memberArray removeAllObjects];
                    [strongSelf.memberArray arrayByAddingObjectsFromArray:strongSelf.tableViewArray];
