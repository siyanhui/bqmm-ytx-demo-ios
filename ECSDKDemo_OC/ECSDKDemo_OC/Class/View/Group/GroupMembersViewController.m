...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSString *deleteAtStr = [NSString stringWithCString:myBuffer encoding:NSUTF8StringEncoding];

    ECGroupMember *member = self.membersArray[indexPath.row];
    NSString *idstr = member.display.length==0?member.memberId:member.display;
    NSString *nameStr = [NSString stringWithFormat:@"%@%@",idstr,deleteAtStr];
    [[NSUserDefaults standardUserDefaults] setObject:nameStr forKey:@"GroupMemberNickName"];
    [[NSUserDefaults standardUserDefaults] synchronize];
    [[DemoGlobalClass sharedInstance].AtPersonArray addObject:member.memberId];
    [self.navigationController popViewControllerAnimated:YES];
}
@end
