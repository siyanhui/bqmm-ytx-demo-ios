        ...
        if (alertView.tag == ChatRoomVIEW_RoomDisslove|| alertView.tag == ChatRoomVIEW_kickOff) {
            ...
            if (buttonIndex == 1) {
                if ([textField.text length]==0) {
                    ...
                    __weak __typeof(self) weakSelf = self;
                    [[ECDevice sharedInstance].meetingManager inviteMembersJoinMultiMediaMeeting:self.curChatroomId andIsLoandingCall:(alertView.tag==ChatRoomVIEW_addmember) andMembers:@[textField.text] andDisplayNumber:nil andSDKUserData:nil andServiceUserData:nil completion:^(ECError *error, NSString *meetingNumber) {
                        __strong __typeof(weakSelf)strongSelf = weakSelf;
                        if (error.errorCode != ECErrorType_NoError) {
                            NSString* detail = error.errorDescription.length>0?[NSString stringWithFormat:@"\r描述:%@",error.errorDescription]:@"";
                            [strongSelf showToast:[NSString stringWithFormat:@"错误码:%d%@",(int)error.errorCode,detail]];
...
- (void)loadView {
    ...
    pointImg.frame = CGRectMake(0.0f, frameY, 320.0f, 29.0f);
    ...
    UIView *listView = [[UIView alloc] initWithFrame:CGRectMake(0.0f, frameY+55.0f, 320.0f, 80.0f)];
    [self.view addSubview:listView];
    membersListView = listView;

    animBackview = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"animation_bg.png"]];
    animBackview.center = CGPointMake(160.0f, frameY+self.view.frame.size.height*0.5);
    ...
    UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(0.0f, self.view.frame.size.height-150.0f-44+frameY, 320.0f, 28.0f)];
    ...
    view1.frame = CGRectMake(0, self.view.frame.size.height-74+frameY, 320, 30);
    ...
    micBtn.frame = CGRectMake(115.0f, self.view.frame.size.height-105.0f+frameY, 89.0f, 61.0f);
...
-(void)createChatroomWithChatroomName:(NSString*)chatroomName andPassword:(NSString *)roomPwd andSquare:(NSInteger)square andKeywords:(NSString *)keywords  andIsAutoClose:(BOOL)isAutoClose andVoiceMod:(NSInteger) voiceMod andAutoDelete:(BOOL) autoDelete andIsAutoJoin:(BOOL) isAutoJoin {
    ...
    [[ECDevice sharedInstance].meetingManager createMultMeetingByType:params completion:^(ECError *error, NSString *meetingNumber) {
        ...
        if (error.errorCode == ECErrorType_NoError && meetingNumber.length > 0) {
            ...
            NSString* detail = error.errorDescription.length>0?[NSString stringWithFormat:@"\r描述:%@",error.errorDescription]:@"";
            UIAlertView *alertview = [[UIAlertView alloc]  initWithTitle:@"创建语音群聊失败" message:[NSString stringWithFormat:@"错误码:%d%@",(int)error.errorCode,detail]  delegate:strongSelf cancelButtonTitle:@"确定" otherButtonTitles:nil];
...
- (void) joinChatroomInRoom:(NSString *)roomNo andPwd:(NSString *)pwd {
    ...
    [[ECDevice sharedInstance].meetingManager joinMeeting:roomNo ByMeetingType:ECMeetingType_MultiVoice andMeetingPwd:pwd completion:^(ECError *error, NSString *meetingNumber) {
        ...
        if (error.errorCode == ECErrorType_NoError && meetingNumber.length > 0) {
            ...
            NSString* detail = error.errorDescription.length>0?[NSString stringWithFormat:@"\r描述:%@",error.errorDescription]:@"";
            UIAlertView *alertview = [[UIAlertView alloc]  initWithTitle:@"加入群聊失败" message:[NSString stringWithFormat:@"错误码:%d%@",(int)error.errorCode,detail]  delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil];
            alertview.tag = ChatRoomVIEW_RoomDisslove;
            [alertview show];
        }
    }];
