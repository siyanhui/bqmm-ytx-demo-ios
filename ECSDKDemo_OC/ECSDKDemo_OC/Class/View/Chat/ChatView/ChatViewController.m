...
#warning 表情页面
    ...
    UIButton *_moreBtn;
//    CustomEmojiView *_emojiView;    BQMM集成 去掉原demo的表情键盘
...
//        toolbarDisplay = ToolbarDisplay_None;
        ...
        //只显示输入view
        frameY = endFrame.origin.y-containerViewFrame.size.height+ToolbarMoreViewHeight;
...
//                frameY = endFrame.origin.y-containerViewFrame.size.height-ToolbarInputViewHeight;
//                break;
//                
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSString *msgType = @"none";
    if (extDic != nil) {
        msgType = extDic[@"txt_msgType"];
    }
    
    NSString *cellidentifier = [NSString stringWithFormat:@"%@_%@_%d_%@", isSender?@"issender":@"isreceiver",NSStringFromClass([message.messageBody class]),(int)fileType, msgType];
...
- (void)growingTextViewDidBeginEditing:(HPGrowingTextView *)growingTextView {
    ...
    if ([self.sessionId hasPrefix:@"g"] && [_inputTextView.text myContainsString:@"@"] && isOpenMembersList && _GroupMemberNickName.length>0) {
        ...
        if (_inputTextView.text.length<arrowLocation) {
            arrowLocation-=1;
        }
